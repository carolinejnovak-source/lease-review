...
- RENT PAYMENT METHOD: VIP standard is ACH/wire ONLY. (a) If the lease requires certified check or physical mail only, replace with "ACH/wire transfer to Landlord's designated bank account." (b) If the lease already allows ACH but ALSO allows mailing/physical check (e.g., "ACH or wire transfer... or mailed to Landlord at the following address"), generate a redline that REMOVES the mailing/physical-payment option — find the ", or mailed to Landlord at the following address" clause (verbatim) and replace it with a period or nothing. Do NOT leave any physical-mail payment option in the proposed language.
